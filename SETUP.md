...
| 열 | A | B | C | D | E |
|----|---|---|---|---|---|
| 헤더 | 간식이름 | 링크 | 예상가격 | 수량 | 활성여부 |
| 예시 | 생수 2L 6팩 | https://... | 5000 | 1 | Y |

- **활성여부**: `Y`이면 매월 자동 삽입, `N`이면 비활성
- 관리자 페이지에서 추가/삭제/비활성화 가능
...
고정품목 시트에 등록된 항목을 관리자가 수동으로 해당 월 추천목록에 추가할 수 있습니다.

1. `/admin` 관리자 페이지 접속
2. "고정 품목 관리" 섹션에서 고정 품목 등록/관리
3. **"이번 달 추천목록에 고정 품목 추가"** 버튼 클릭
4. 이미 해당 월에 같은 이름의 품목이 있으면 중복 추가되지 않습니다

> 트리거 설정 없이 관리자 페이지에서 직접 조작합니다.
...
### 월간 프로세스
1. **월초**: 관리자가 `/admin`에서 "고정 품목 추가" 버튼 클릭 (좋아요 0으로 초기화)
2. **월초**: 연구실원들이 `/home`에서 간식 추천 & 좋아요
...
- **데이터 안 보임**: 스프레드시트 시트 이름이 정확히 `추천목록`, `구매기록`, `설정`, `고정품목`인지 확인
